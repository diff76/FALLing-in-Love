...
gen_conn() { local i="$1" s="$2" e="$3" out="$W/conn_$1" try=1
  ...
  while [ $try -le 3 ]; do
    ...
    higgsfield generate create "$VMODEL" --prompt "$(cat "$PR/conn_$i.txt")" --start-image "$s" --end-image "$e" \
      $VOPTS --aspect_ratio 16:9 --duration "$CONN_DUR" --wait --wait-timeout 20m --json > "$out.json" 2> "$out.err"
