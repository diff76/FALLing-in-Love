...
gen_conn() { local i="$1" s="$2" e="$3" out="$W/conn_$1" try=1
  if [ -s "$out.mp4" ]; then log "conn $i present — skip"; return 2; fi   # 2 = already there (callers must NOT post-process again)
  while [ $try -le 3 ]; do
...
for n in $NAMES; do
  if [ -n "$prev" ]; then i=$((i+1))
    if [ "$n" = "one-more-song" ]; then
      ...
      # Only a FRESH render gets reversed — a skipped (present) file was reversed when it was made.
      ( gen_conn "$i" "$W/first_$n.png" "$W/last_$prev.png"; rc=$?; [ $rc -eq 0 ] && ffmpeg -v error -y -i "$W/conn_$i.mp4" -vf reverse -an "$W/conn_${i}_fwd.mp4" && mv "$W/conn_${i}_fwd.mp4" "$W/conn_$i.mp4" && log "conn $i time-reversed" ) &
    else
      [ -f "$W/last_$prev.png" ] && [ -f "$W/first_$n.png" ] && ( gen_conn "$i" "$W/last_$prev.png" "$W/first_$n.png" || true ) &
    fi
    sleep 3
  fi
  prev="$n"
