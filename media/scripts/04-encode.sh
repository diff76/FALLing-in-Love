...
PY2
  if [ $? -eq 3 ]; then log "conn 7 starts on the living room — reversing"; ffmpeg -v error -y -i "$W/conn_7.mp4" -vf reverse -an "$W/_c7r.mp4" && mv "$W/_c7r.mp4" "$W/conn_7.mp4"; enc "$W/conn_7.mp4" "$OUT/conn7.mp4"; fi
fi
