...
- Phones scrub IMAGE SEQUENCES, not video (2026-09-23): iOS WebKit (Safari and Chrome alike) caps live video decoders, suspends off-screen media and never paints a seek on an element that was not played by a gesture — on real iPhones the video chain froze or showed posters only. `media/scripts/05-frames-m.sh` extracts every portrait dive/connector to `apps/web/public/media/frames/<id>/fNNN.webp` (10 fps, 540 px wide, ≈40 MB total, ≈3 MB per dive) and the manifest publishes `framesMobile` / `connectorsFramesMobile`; the engine draws the nearest loaded frame on a canvas, loads a screen ahead and releases segments two screens behind. The `-m.mp4` encodes stay as a fallback for phones without frame sets. Open the page with `#swdebug` for a per-segment state overlay.
